...
it.only('proxy interception', () => {
	let cacheInvalidations = 0;
	const invalidateCache = () => ++cacheInvalidations;

	const dom = new JSDOM('<div>HTMLElement</div>', { runScripts: 'outside-only' });

	const modifiers = [
		[ dom.window.Element.prototype, 'innerHTML' ],
		[ dom.window.Node.prototype, 'appendChild' ]
	];
	for (const [ target, ...properties ] of modifiers) {
		for (const property of properties) {
			const original = Object.getOwnPropertyDescriptor(target, property);
			const intercepted = { ...original };
			for (const i of [ 'set', 'value' ]) {
				if (!original[i]) continue;
				intercepted[i] = function painterInvalidateCacheOnDOMModification (...args) {
					invalidateCache();
					original[i].call(this, ...args);
				};
				break;
			}

			Object.defineProperty(target, property, intercepted);
		}
	}

	const element = dom.window.document.querySelector('div');

	assert.equal(element.innerHTML, 'HTMLElement');
	...
	element.appendChild(dom.window.document.createElement('span'));
